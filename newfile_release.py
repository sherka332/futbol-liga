...
# ---------------- DIRECT MESSAGES ----------------
@app.get("/api/dm/<int:telegram_id>")
def get_dm(telegram_id):
    user,err=require_user()
    if err:return err
    if telegram_id == user["id"]:
        return jsonify({"ok":False,"error":"O'zingizga xabar yubora olmaysiz"}),400
    con=db()
    other=con.execute("""SELECT u.telegram_id,u.username,u.first_name,t.name team_name
                         FROM users u LEFT JOIN teams t ON t.id=u.team_id
                         WHERE u.telegram_id=? AND u.active=1""",(telegram_id,)).fetchone()
    if not other:
        con.close();return jsonify({"ok":False,"error":"Foydalanuvchi topilmadi"}),404
    rows=con.execute("""SELECT id,sender_id,receiver_id,text,created_at FROM direct_messages
                       WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)
                       ORDER BY id ASC LIMIT 200""",(user["id"],telegram_id,telegram_id,user["id"])).fetchall()
    con.close()
    return jsonify({"ok":True,"other":dict(other),"messages":[dict(r) for r in rows]})

@app.post("/api/dm/<int:telegram_id>")
def send_dm(telegram_id):
    user,err=require_user()
    if err:return err
    if telegram_id == user["id"]:
        return jsonify({"ok":False,"error":"O'zingizga xabar yubora olmaysiz"}),400
    ...
    if len(text)>1000:return jsonify({"ok":False,"error":"Xabar juda uzun"}),400
    con=db()
    other=con.execute("SELECT telegram_id FROM users WHERE telegram_id=? AND active=1",(telegram_id,)).fetchone()
    if not other:
        con.close();return jsonify({"ok":False,"error":"Foydalanuvchi topilmadi"}),404
    con.execute("INSERT INTO direct_messages(sender_id,receiver_id,text) VALUES(?,?,?)",(user["id"],telegram_id,text))
    con.commit();con.close();return jsonify({"ok":True})
...
def telegram_api(method, data=None):
    ...
    req=Request(f"https://api.telegram.org/bot{BOT_TOKEN}/{method}", data=payload, headers={"Content-Type":"application/x-www-form-urlencoded"})
    with urlopen(req, timeout=35) as r:
        return json.loads(r.read().decode("utf-8"))

def bot_polling():
    if not BOT_TOKEN or not WEB_APP_URL:
        print("Telegram bot polling disabled: BOT_TOKEN or WEB_APP_URL missing")
        return
    offset=0
    while True:
        try:
            res=telegram_api("getUpdates", {"timeout":25, "offset":offset})
            for upd in res.get("result",[]):
                ...
                if text.startswith("/start"):
                    kb={"inline_keyboard":[[{"text":"⚽ eFootball Ligani ochish 🚀","web_app":{"url":WEB_APP_URL}}]]}
                    telegram_api("sendMessage", {"chat_id":chat["id"],"text":"⚽ eFootball La Liga\n\n20 jamoa • 38 tur • Jonli turnir jadvali\n\nLigaga kirish uchun quyidagi tugmani bosing:","reply_markup":json.dumps(kb,ensure_ascii=False)})
                elif text.startswith("/app"):
                    kb={"inline_keyboard":[[{"text":"⚽ Ligani ochish 🚀","web_app":{"url":WEB_APP_URL}}]]}
                    telegram_api("sendMessage", {"chat_id":chat["id"],"text":"Ligani ochish:","reply_markup":json.dumps(kb,ensure_ascii=False)})
                elif text.startswith("/help"):
                    telegram_api("sendMessage", {"chat_id":chat["id"],"text":"/start — eFootball Ligani ochish\n/app — Mini Appni ochish"})
...
if BOT_TOKEN and WEB_APP_URL:
    threading.Thread(target=bot_polling, daemon=True, name="telegram-bot").start()
