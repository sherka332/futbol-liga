...
def init_db():
    ...
    con.execute("CREATE INDEX IF NOT EXISTS idx_direct_messages_pair ON direct_messages(sender_id, receiver_id, id)")
...
def get_direct_messages(other_id):
    ...
    if err: return err
    if other_id == user["id"]:
        return jsonify({"ok": False, "error": "O'zingiz bilan chat qilib bo'lmaydi"}), 400
    con = db()
    me = user_row(con, user["id"])
    other = user_row(con, other_id)
    if not me or not me["active"] or not other or not other["active"]:
        con.close(); return jsonify({"ok": False, "error": "Ishtirokchi topilmadi yoki faol emas"}), 404
    rows = con.execute("""SELECT d.id,d.sender_id,d.receiver_id,d.text,d.created_at,
        su.username sender_username,su.first_name sender_first_name,
        ru.username receiver_username,ru.first_name receiver_first_name
        FROM direct_messages d
        LEFT JOIN users su ON su.telegram_id=d.sender_id
        LEFT JOIN users ru ON ru.telegram_id=d.receiver_id
        WHERE (d.sender_id=? AND d.receiver_id=?) OR (d.sender_id=? AND d.receiver_id=?)
        ORDER BY d.id ASC""", (user["id"], other_id, other_id, user["id"])).fetchall()
    target = {"telegram_id": other["telegram_id"], "username": other["username"], "first_name": other["first_name"],
              "team_name": other["team_name"], "logo": team_logo(other["team_name"]) if other["team_name"] else ""}
    con.close()
    return jsonify({"ok": True, "target": target, "messages": [{
        "id": r["id"], "sender_id": r["sender_id"], "receiver_id": r["receiver_id"], "text": r["text"],
...
def send_direct_message(other_id):
    ...
    if err: return err
    if other_id == user["id"]:
        return jsonify({"ok": False, "error": "O'zingizga xabar yuborib bo'lmaydi"}), 400
    body = request.get_json(silent=True) or {}
...
@app.get("/chat.js")
def chat_script():
    return send_from_directory(BASE_DIR, "chat.js", mimetype="application/javascript")
