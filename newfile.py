...
def init_db():
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS achievements (
        ...
        season INTEGER NOT NULL, place INTEGER NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP,
...
def finalize_due_matches(con):
    """Deadline o'tgan va hali rasmiy bo'lmagan o'yinlarni avtomatik 0:0 qiladi."""
    now = now_local()
    rows = con.execute("""SELECT m.id, m.status, r.round_date, r.deadline
    ...
    for r in rows:
        if now >= deadline_dt(r["round_date"], r["deadline"]):
            # Deadline qoidasi: pending natija bo'lsa aynan shu natija avtomatik tasdiqlanadi;
            # hech kim kiritmagan bo'lsa 0:0.
            if r["status"] == "pending":
                full = con.execute("SELECT pending_home_goals,pending_away_goals FROM matches WHERE id=?", (r["id"],)).fetchone()
                hg = full["pending_home_goals"] if full and full["pending_home_goals"] is not None else 0
                ag = full["pending_away_goals"] if full and full["pending_away_goals"] is not None else 0
            else:
                hg, ag = 0, 0
            con.execute("""UPDATE matches SET home_goals=?, away_goals=?, status='auto',
                         pending_home_goals=NULL, pending_away_goals=NULL, submitted_by=NULL,
                         played_at=? WHERE id=?""", (hg, ag, now.isoformat(), r["id"]))
            changed = True
    if changed:
        con.commit()
...
def admin_generate_rounds():
    ...
    con.commit();con.close();return jsonify({"ok":True,"rounds_created":len(pairs),"teams":len(teams)})
