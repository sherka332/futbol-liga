...
def finalize_due_matches(con):
    ...
    for r in rows:
        if now >= deadline_dt(r["round_date"], r["deadline"]):
            con.execute("""UPDATE matches SET home_goals=0, away_goals=0,
                         status='auto', pending_home_goals=NULL,
                         pending_away_goals=NULL, submitted_by=NULL,
                         played_at=? WHERE id=?""", (now.isoformat(), r["id"]))
            changed = True
    if changed:
        con.commit()
...
# Yangi mavsum funksiyasi
@app.post("/api/admin/start_next_season")
def admin_start_next_season():
    _, err = require_admin()
    if err:
        return err
    con = db()
    try:
        ...
        row = con.execute("SELECT value FROM settings WHERE key='season'").fetchone()
        current = int(row["value"]) if row else 1
        con.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('season',?)", (str(current + 1),))
        con.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('season_finished','0')")
        con.commit()
        return jsonify({"ok": True, "season": current + 1})
    finally:
        con.close()
...
def admin_generate_rounds():
    ...
    for idx,pairset in enumerate(pairs,1):
