...
def build_standings(con):
    ...
    for t in rows:
        ms=con.execute("""SELECT home_team,away_team,home_goals,away_goals FROM matches
                         WHERE status IN ('official','auto') AND (home_team=? OR away_team=?)""",
                       (t["id"],t["id"])).fetchall()
        ...
        for m in ms:
            ...
            if a > b:
                ...
                losses += 1
        owner=con.execute("""SELECT telegram_id,username,first_name FROM users
                             WHERE team_id=? AND active=1""",(t["id"],)).fetchone()
        result.append({
...
def admin_update_match():
    ...
    con=db();con.execute("""UPDATE matches SET home_goals=?,away_goals=?,status='official',played_at=?,scheduled_at=COALESCE(?,scheduled_at),
        pending_home_goals=NULL,pending_away_goals=NULL,submitted_by=NULL WHERE id=?""",(hg,ag,now_local().isoformat(),b.get("scheduled_at"),mid));con.commit();finish_season_if_needed(con);con.close();return jsonify({"ok":True})
...
def finish_season_if_needed(con):
    ...
    finished_matches=con.execute("""SELECT COUNT(*) c FROM matches
                                    WHERE status IN ('official','auto')""").fetchone()["c"]
    finished_row=con.execute("SELECT value FROM settings WHERE key='season_finished'").fetchone()
    already_finished=finished_row and finished_row["value"]=="1"

    if total_rounds != 38 or total_matches == 0 or finished_matches != total_matches or already_finished:
        return False

    season=int(con.execute("SELECT value FROM settings WHERE key='season'").fetchone()["value"])
    table=build_standings(con)

    for row in table[:3]:
        owner=con.execute("""SELECT telegram_id FROM users
                             WHERE team_id=? AND active=1""",(row["id"],)).fetchone()
        if owner:
            con.execute("""INSERT OR IGNORE INTO achievements
                (telegram_id,season,place,team_id,team_name)
                VALUES(?,?,?,?,?)""",
                (owner["telegram_id"],season,row["rank"],row["id"],row["name"]))
...
def season_winners():
    ...
    rows=con.execute("""SELECT a.place,a.telegram_id,a.team_name,a.team_id,
                               u.username,u.first_name
                        FROM achievements a
                        LEFT JOIN users u ON u.telegram_id=a.telegram_id
                        WHERE a.season=?
                        ORDER BY a.place ASC""",(season,)).fetchall()
    winners=[]
    for r in rows:
        team_name=r["team_name"]
        if not team_name and r["team_id"]:
            tr=con.execute("SELECT name FROM teams WHERE id=?",(r["team_id"],)).fetchone()
            team_name=tr["name"] if tr else "Jamoa"
        winners.append({
            "place":r["place"],
            "telegram_id":r["telegram_id"],
            "team":team_name or "Jamoa",
            "logo":team_logo(team_name) if team_name else "",
            "username":r["username"],
            "name":r["first_name"] or r["username"] or "Ishtirokchi"
        })
    con.close()
...
def participants():
    con=db()
    rows=con.execute("""SELECT u.telegram_id,u.username,u.first_name,u.active,
                               t.id team_id,t.name team_name
                        FROM users u
                        LEFT JOIN teams t ON t.id=u.team_id
                        WHERE u.active=1
                        ORDER BY t.name,u.first_name""").fetchall()
    data=[]
    for r in rows:
        ach=con.execute("""SELECT place,COUNT(*) c FROM achievements
                           WHERE telegram_id=? GROUP BY place""",(r["telegram_id"],)).fetchall()
        cups={str(x["place"]):x["c"] for x in ach}
        team_name=r["team_name"]
        data.append({
            "telegram_id":r["telegram_id"],
            "username":r["username"],
...
def public_profile(telegram_id):
    ...
    cups_rows=con.execute("""SELECT place,COUNT(*) c FROM achievements
                             WHERE telegram_id=? GROUP BY place""",(telegram_id,)).fetchall()
    cups={str(r["place"]):r["c"] for r in cups_rows}

    stats={"played":0,"wins":0,"draws":0,"losses":0,"gf":0,"ga":0,"gd":0,"points":0}
    matches=[]
    if team_id:
        rows=con.execute("""SELECT m.*,r.round_no,r.round_date,r.deadline,
                                    ht.name home_name,at.name away_name
                             FROM matches m
                             ...
                             ORDER BY r.round_no,m.id""",(team_id,team_id)).fetchall()
        for m in rows:
            matches.append({
                "id":m["id"],"round_no":m["round_no"],"round_date":m["round_date"],
...
def start_next_season():
    ...
    if not finished:
        con.close()
        return jsonify({"ok":False,"error":"Avval 38 turdagi barcha o'yinlar yakunlanishi kerak"}),409
